...
export class Scrape {
  ...
  doClick(elem: string): Promise<void> {
    return new Promise(async (resolve, reject) => {
      try {
        await Promise.all([
          // wait for loading
          await Scrape.doWaitSelector(elem, 3000),
          // wait for navigation
          await Scrape.doWaitNav(),
  ...
  doSingleEval(selector: string, property: string): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (item !== null) {
          // wait for loading
          await Scrape.doWaitSelector(selector, 3000);
          // got data
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (const ls of list) {
          // push to data set
          data.push(await (await ls.getProperty(property)).jsonValue());
        }
        // resolved
        resolve(data);
  ...
  // waitSelector
  static doWaitSelector(elem: string, time: number): Promise<void> {
    return new Promise(async (resolve, reject) => {
      try {
...
const getChromePath = (): Promise<string> => {
  return new Promise(async (resolve, reject) => {
    // chrome tmp path
    ...
    if (fs.existsSync(CHROME_EXEC_PATH1)) {
      resolve(CHROME_EXEC_PATH1 ?? '');

      // 64bit
    } else if (fs.existsSync(CHROME_EXEC_PATH2)) {
      resolve(CHROME_EXEC_PATH2 ?? '');
